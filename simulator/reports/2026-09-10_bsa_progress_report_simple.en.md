...
| context | same scans with 2–4× longer chunks (DOM 16–128, other 128–512) | queued (5 runs) |
| model | β measured on Qwen3-VL-8B, Qwen3-8B, OLMo-2-7B, SmolLM2-1.7B; α / A scan on Qwen3-VL-8B in sglang | 8B in sglang: full attention 60, (16, 0.25) 40, α = 0.125 → 36, α = 0.5 → 43 (A = 1 / 64 running); β measurements running |
...
Early reading: the exponent optimum is confirmed on unseen tasks and the anchor has no effect there either (A = 64 went from −8 to +3), so the anchor claim is now simply "no effect between 1 and 64". The 8B model behaves differently — sparsity at 1,024 tokens costs it 20 steps against full attention (3 for the 30B), and α = 0.5 is at least as good as 0.25 — which is what the theory predicts if its key-growth exponent β is higher (RoPE base 5×10⁶ vs 10⁶); the β measurement is the pending piece. Remaining: geometry scan (5 runs), 8B anchor runs (2), β (4 models), ~3 hours.
